...
class TestCasesReviewMeeting(Meetings):
    version_map = {
        "0.0.1": "unify"
    }
    ...
    def unify(self, workspace, **kwargs):
        ...
        prompt["Context"] = self._get_information_from_log(workspace, "Test Cases")
        review = openai_api.execute("default_json_prompt", prompt=json.dumps(prompt))["content"]
        self._write_log(workspace, 'Test Cases Review', review, prompt=json.dumps(prompt))
        return review
